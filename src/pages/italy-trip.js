  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!zapierWebhookUrl) {
        ...
        formBody.append('destination', formData.destination); // Will send 'ايطاليا'
        const now = new Date();
        const date = now.toLocaleDateString();
        const time = now.toLocaleTimeString();
        formBody.append('formName', 'Italy Trip Form'); // Updated form name
    ...
    try {
      const emailResponse = await fetch('/api/send-lead-email', {
        ...
        body: JSON.stringify({
          ...
          destination: formData.destination, // Will send 'ايطاليا'
          formName: 'Italy Trip Form', // Updated form name
