...
export default function BosniaTrip() {
  ...
  const [formData, setFormData] = useState({
    ...
    destination: 'البوسنة', // Changed destination to Bosnia
  });
      ...
      const response = await fetch('/api/fb-events', {
        ...
        body: JSON.stringify({
          ...
          custom_data: {
            ...
            destination: data.destination, // This will be 'البوسنة' from state
            full_name: data.name,
          },
  ...
  const handleSubmit = async (e) => {
    ...
    const eventData = {
      content_name: 'Bosnia Trip Form', // Updated form name
    ...
    try {
      ...
      if (!zapierWebhookUrl) {
        ...
        formBody.append('destination', formData.destination); // Will send 'البوسنة'
        const now = new Date();
        const date = now.toLocaleDateString();
        const time = now.toLocaleTimeString();
        formBody.append('formName', 'Bosnia Trip Form'); // Updated form name
    ...
    try {
      const emailResponse = await fetch('/api/send-lead-email', {
        ...
        body: JSON.stringify({
          ...
          destination: formData.destination, // Will send 'البوسنة'
          formName: 'Bosnia Trip Form', // Updated form name
    ...
    setFormData({
      ...
      email: '',
      nationality: '',
      destination: 'البوسنة', // Updated destination in reset
    });
