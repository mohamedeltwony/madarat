      ...
      const response = await fetch('/api/fb-events', {
        ...
        body: JSON.stringify({
          ...
          custom_data: {
            ...
            destination: data.destination, // This will be 'تركيا' from state
            full_name: data.name,
          },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!zapierWebhookUrl) {
        ...
        formBody.append('destination', formData.destination); // Will send 'تركيا'
        const now = new Date();
    ...
    try {
      const emailResponse = await fetch('/api/send-lead-email', {
        ...
        body: JSON.stringify({
          ...
          destination: formData.destination, // Will send 'تركيا'
