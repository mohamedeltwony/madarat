...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!accessToken) {
      console.error('[API /send-lead-email] FACEBOOK_ACCESS_TOKEN environment variable is not set. Facebook Conversion API event not sent.');
    } else {
      const eventData = {
        event_name: 'Lead',
        event_time: Math.floor(Date.now() / 1000), // Current timestamp in seconds
        user_data: {
          // Facebook strongly recommends hashing sensitive data like email and phone using SHA256
          // before sending it via the Conversion API for privacy.
          // For production, implement a hashing function here (e.g., using 'crypto' module).
          em: email ? email.toLowerCase() : undefined, // Email (lowercase)
          ph: phone ? phone.replace(/\D/g, '') : undefined, // Phone (digits only)
          client_ip_address: ipAddress,
          client_user_agent: userAgentString,
          fbc: fbc, // Facebook Click ID
          fbp: fbp, // Facebook Browser ID
          // Add other user data if available and relevant (e.g., external_id, subscription_id)
        },
        // Add custom data if relevant for reporting/segmentation, e.g., value, currency, content_name, content_category
        // custom_data: {
        //   value: 0, // Example: Assign a value to the lead
        //   currency: 'SAR', // Example: Currency
        //   destination: destination, // Example: Include destination as custom data
        //   form_name: formName, // Example: Include form name
        // },
        // context: { // Optional: Include context data
        //   page: {
        //     url: pageUrl,
        //   },
        //   // Add other context data if needed
        // },
        event_source_url: pageUrl, // URL where the event occurred
        action_source: 'website', // Source of the event
      };

      console.log('[API /send-lead-email] Sending Lead event to Facebook Conversion API...');
      ...
      try {
        ...
        if (response.ok) {
          console.log('[API /send-lead-email] Facebook Conversion API Lead event sent successfully:', result);
        } else {
          console.error('[API /send-lead-email] Failed to send Facebook Conversion API Lead event:', result);
          console.error('[API /send-lead-email] Facebook API Error Details:', result.error);
        }
      } catch (apiError) {
        console.error('[API /send-lead-email] Error sending Facebook Conversion API Lead event:', apiError);
      }
    }
    // --- End Facebook Conversion API Event ---
