...
export default function Home({ posts, destinations, pagination }) {
...
export async function getStaticProps() {
  ...
  const { categories } = await getCategories({
    hierarchical: true,
    include: ['featuredImage', 'parent'],
    per_page: 6,
  });
  
  // Filter categories to get only destinations (those without a parent)
  const destinations = categories.filter(category => !category.parent);
