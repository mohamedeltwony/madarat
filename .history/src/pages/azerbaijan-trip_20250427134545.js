...
export default function azerbaijan() {
  ...
  const [formData, setFormData] = useState({
    ...
    destination: 'اذربيجان', // Changed destination
  });
      ...
      const response = await fetch('/api/fb-events', {
        ...
        body: JSON.stringify({
          ...
          custom_data: {
            ...
            destination: data.destination, // This will be 'اذربيجان' from state
            full_name: data.name,
          },
  ...
  const handleSubmit = async (e) => {
    ...
    const eventData = {
      content_name: 'azerbaijan Trip Form',
    ...
    try {
      ...
      if (!zapierWebhookUrl) {
        ...
        formBody.append('destination', formData.destination); // Will send 'اذربيجان'
        const now = new Date();
        const date = now.toLocaleDateString();
        const time = now.toLocaleTimeString();
        formBody.append('formName', 'azerbaijan Trip Form');
    ...
    try {
      const emailResponse = await fetch('/api/send-lead-email', {
        ...
        body: JSON.stringify({
          ...
          destination: formData.destination, // Will send 'اذربيجان'
          formName: 'azerbaijan Trip Form',
  ...
  const features = [
    ...
    { text: 'الرخصة الدولية' }, // No direct icon match
    { text: 'دليل سياحي كامل' }, // No direct icon match
