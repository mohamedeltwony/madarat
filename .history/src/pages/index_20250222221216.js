...
export async function getStaticProps() {
  ...
  // Fetch destinations from WordPress
  const response = await fetch(`${process.env.WORDPRESS_API_URL}/wp-json/wp/v2/destinations`);
  ...
  const destinations = destinationsData.map(destination => ({
    ...
    tripCount: destination.trip_count || 0
  }));
